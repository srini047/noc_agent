...
stop_at() {
  printf "\n${RED}${BOLD}Validation stopped at %s.${NC} Fix before continuing.\n" "$1"; exit 1
}
...
if [ "$HTTP_CODE" = "200" ]; then
  ...
  fail "HF Space not reachable"; stop_at "Step 1"
else
  fail "HF Space /reset returned HTTP $HTTP_CODE"; stop_at "Step 1"
fi
...
if ! command -v openenv &>/dev/null; then
  fail "openenv not found"; stop_at "Step 3"
fi
